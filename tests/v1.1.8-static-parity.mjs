...
for(const i of [17,19,20,21]){const r=`stage${i}-client.js`;add(`Stage ${i} full client exact v1.1.7 donor`,sha(r)===golden[r],sha(r))}
add('Stage 18 intentionally extended only for governed Ask Atlas context',sha('stage18-client.js')==='6e269df37787cb835c9af267dbda0860b7f0bc66c606a55a82d7377bbc315594',sha('stage18-client.js'));
for(const f of ['stage24-page0-rules.js','stage24-page0-composer.js','stage24-enterprise-entry.js'])add(`v1.1.8 sidecar present · ${f}`,fs.existsSync(f)&&index.includes(`<script src=\"${f}\"></script>`));
