...
/**
 * v1.1.6 strict LIVE browser certification.
 *
...
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'scoip-v116-chrome-'));
...
async function certify(name,viewport){
  ...
  const expr=`(()=>({
    ...
    brand:document.querySelector('.brand small')?.textContent||'',
  ...
  check(/v1\.1\.6/.test(d.brand||''),`${name}: release identity v1.1.6 visible · ${d.brand}`);
  const shot=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});
  fs.mkdirSync('audits/v1.1.6',{recursive:true});
  fs.writeFileSync(`audits/v1.1.6/v1.1.6-${name}-certified.png`,Buffer.from(shot.data,'base64'));
  ws.close();
...
try{
  ...
  fs.mkdirSync('audits/v1.1.6',{recursive:true});
  fs.writeFileSync('audits/v1.1.6/live-browser-certificate.json',JSON.stringify({release:'v1.1.6',certifiedAt:new Date().toISOString(),chromium:exe,desktop,mobile,failures},null,2));
...
console.log(failures===0?'PASS · v1.1.6 strict live browser certificate · release gate satisfied':`FAIL · v1.1.6 strict live browser certificate · ${failures} check(s) failed`);
