...
/**
 * v1.1.7 strict LIVE browser + reference-access certification.
 *
...
const tmp=fs.mkdtempSync(path.join(os.tmpdir(),'scoip-v117-chrome-'));
...
async function certify(name,viewport){
  ...
  const expr=`(()=>({
    ...
    brand:document.querySelector('.brand small')?.textContent||'', reference:{button:!!document.getElementById('navReference117'),api:!!window.AtlasReference117},
  ...
  check(/v1\.1\.7/.test(d.brand||''),`${name}: release identity v1.1.7 visible · ${d.brand}`);
  ...
  fs.mkdirSync('audits/v1.1.7',{recursive:true});
  fs.writeFileSync(`audits/v1.1.7/v1.1.7-${name}-certified.png`,Buffer.from(shot.data,'base64'));
  ws.close();
...
try{
  ...
  fs.mkdirSync('audits/v1.1.7',{recursive:true});
  fs.writeFileSync('audits/v1.1.7/live-browser-certificate.json',JSON.stringify({release:'v1.1.7',certifiedAt:new Date().toISOString(),chromium:exe,desktop,mobile,failures},null,2));
...
console.log(failures===0?'PASS · v1.1.7 strict live browser/reference-access certificate · release gate satisfied':`FAIL · v1.1.7 strict live browser/reference-access certificate · ${failures} check(s) failed`);
