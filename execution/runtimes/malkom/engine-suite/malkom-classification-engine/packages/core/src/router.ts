...
import { z } from 'zod';
import { ClassificationEngine, EngineConflictError, EngineNotFoundError, EngineValidationError } from './engine.js';
import { configBundleSchema, modelDefinitionSchema } from './schemas.js';
...
export const buildFetchHandler = (engine: ClassificationEngine, keys: AuthKeys) => {
  return async (request: Request): Promise<Response> => {
    const url = new URL(request.url);
    const path = url.pathname.replace(/\/$/, '') || '/';
    const method = request.method.toUpperCase();

    if (path === '/v1/health') return json(200, { ok: true, engine: 'malkom-classify', version: '0.1.0' });
    ...
    try {
      if (path === '/v1' && method === 'GET') {
        ...
        return json(200, {
          engine: 'malkom-classify',
          version: '0.1.0',
          capabilities: { algorithms: ['ONLINE_NB'] },
      ...
      if (path === '/v1/schemas' && method === 'GET') {
        ...
        if (denied) return denied;
        return json(200, {
          'model-definition': z.toJSONSchema(modelDefinitionSchema, { io: 'input' }),
      ...
      if (path === '/v1/models' && method === 'GET') {
        const denied = need('read');
        if (denied) return denied;
        return json(200, { models: engine.listModels() });
      }
      ...
      const modelMatch = path.match(/^\/v1\/models\/([A-Za-z0-9][A-Za-z0-9_-]{0,63})(\/(predict|learn))?$/);
      if (modelMatch) {
        const modelId = modelMatch[1] as string;
        const action = modelMatch[3];
        if (action === 'predict' && method === 'POST') {
          const denied = need('read');
          if (denied) return denied;
          return json(200, engine.predict(modelId, await body()));
        }
        if (action === 'learn' && method === 'POST') {
          const denied = need('admin');
          if (denied) return denied;
          return json(200, engine.learn(modelId, await body()));
        }
      ...
      if (path === '/v1/decisions' && method === 'GET') {
        ...
        return json(200, { decisions: engine.decisions(modelId, limit, offset) });
      }
