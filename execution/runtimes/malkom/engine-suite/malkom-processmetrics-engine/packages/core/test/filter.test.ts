/**
 * Ported from the rules engine's filter suite — this file is the drift guard
 * for the condition semantics the engine family shares (null handling,
 * driver coercions, schema safety).
 */
...
describe('scalarEquals driver coercions', () => {
  it('a Date row value equals an ISO string or epoch naming the same instant', () => {
    const d = new Date('2026-08-12T05:00:00Z');
    expect(scalarEquals(d, '2026-08-12T05:00:00Z')).toBe(true);
    expect(scalarEquals(d, '2026-08-12T05:00:00.000Z')).toBe(true); // precision must not matter
    expect(scalarEquals(d, '2026-08-12T10:30:00+05:30')).toBe(true); // same instant, offset form
    expect(scalarEquals(d, d.getTime())).toBe(true);
    expect(scalarEquals(d, '2026-08-12T05:00:01Z')).toBe(false);
    expect(scalarEquals(d, true)).toBe(false);
    expect(scalarEquals(d, null)).toBe(false);
  });

  it('number-vs-numeric-string coercion is symmetric', () => {
    expect(scalarEquals('5', 5)).toBe(true); // string row, number config
    expect(scalarEquals(5, '5')).toBe(true); // number row, string config
    expect(scalarEquals(5, '6')).toBe(false);
    expect(scalarEquals(0, '')).toBe(false); // empty string is no number
    expect(scalarEquals(5, 'x')).toBe(false);
  });
});
...
describe('matches (regex)', () => {
  ...
  it('compileRegex bounds and compiles; junk comes back null', () => {
    expect(compileRegex('^[A-Z]+$')).toBeInstanceOf(RegExp);
    expect(compileRegex('(')).toBeNull();
    expect(compileRegex('')).toBeNull();
    expect(compileRegex('x'.repeat(REGEX_PATTERN_MAX + 1))).toBeNull();
  });
