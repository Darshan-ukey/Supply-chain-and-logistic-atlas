...
function safeDecompositionSummary(entry,taskId){
  if(!entry.decompositionSummaryPath)return null;
  const summary=readJson(entry.decompositionSummaryPath);
  if(summary.classification!=='PUBLIC_SAFE_SUMMARY_ONLY'||summary.moduleId!==entry.moduleId||String(summary.moduleVersion)!==String(entry.moduleVersion)){
    const e=new Error(`Registered decomposition summary is invalid for ${entry.sourceKey}`);e.status=500;throw e;
  }
  const task=(summary.tasks||[]).find(x=>String(x.taskId)===String(taskId));
  if(!task){const e=new Error(`Decomposition summary is missing exact task: ${taskId}`);e.status=500;throw e}
  return {
    status:summary.status,
    contractVersion:summary.contractVersion,
    workUnitCount:Number(task.workUnitCount||0),
    leafCount:Number(task.leafCount||0),
    executorReadyLeafCount:Number(task.executorReadyLeafCount||0),
    blockedByClientBindingLeafCount:Number(task.blockedByClientBindingLeafCount||0),
    blockedByKnowledgeGapLeafCount:Number(task.blockedByKnowledgeGapLeafCount||0),
    independentExecutorProofStatus:task.executorProof||summary.independentExecutorProofStatus||'PENDING',
    detailIncluded:false
  };
}
function applySafeDecompositionSummary(entry,taskId,projection){
  const summary=safeDecompositionSummary(entry,taskId);
  if(!summary)return projection;
  const out=clone(projection)||{};
  out.protectedExecution=out.protectedExecution||{};
  out.protectedExecution.workDecomposition=summary;
  if(out.executionReadiness){
    out.executionReadiness.decompositionStatus='COMPILED_WITH_EXPLICIT_BLOCKERS';
    out.executionReadiness.independentExecutorProofStatus=summary.independentExecutorProofStatus;
    out.executionReadiness.downstream=out.executionReadiness.downstream||{};
    out.executionReadiness.downstream.workDecompositionStatus='COMPILED_WITH_EXPLICIT_BLOCKERS';
    out.executionReadiness.downstream.workDefinitionStatus=out.executionReadiness.downstream.workDefinitionStatus||'NOT_YET_COMPILED';
  }
  return out;
}
// P6.2 PUBLIC_SAFE WorkDefinition summary.
// Allowlisted counts/status only. No definition identity, title, action, gate,
// evidence, source reference or blocker identifier is exposed.
function safeWorkDefinitionSummary(entry,taskId){
  if(!entry.workDefinitionSummaryPath)return null;
  const summary=readJson(entry.workDefinitionSummaryPath);
  if(summary.classification!=='PUBLIC_SAFE_SUMMARY_ONLY'||summary.moduleId!==entry.moduleId||String(summary.moduleVersion)!==String(entry.moduleVersion)){
    const e=new Error(`Registered WorkDefinition summary is invalid for ${entry.sourceKey}`);e.status=500;throw e;
  }
  const task=(summary.tasks||[]).find(x=>String(x.taskId)===String(taskId));
  if(!task){const e=new Error(`WorkDefinition summary is missing exact task: ${taskId}`);e.status=500;throw e}
  return {
    status:summary.status,
    contractVersion:summary.contractVersion,
    workDefinitionCount:Number(task.workDefinitionCount||0),
    compiledLeafCount:Number(task.compiledLeafCount||0),
    notCompiledLeafCount:Number(task.notCompiledLeafCount||0),
    blockedByClientBindingLeafCount:Number(task.blockedByClientBindingLeafCount||0),
    blockedByKnowledgeGapLeafCount:Number(task.blockedByKnowledgeGapLeafCount||0),
    coverageStatus:String(task.coverageStatus||''),
    independentExecutorProofStatus:task.executorProof||summary.independentExecutorProofStatus||'NOT_INDEPENDENTLY_PROVEN',
    detailIncluded:false
  };
}
function applySafeWorkDefinitionSummary(entry,taskId,projection){
  const summary=safeWorkDefinitionSummary(entry,taskId);
  if(!summary)return projection;
  const out=clone(projection)||{};
  out.protectedExecution=out.protectedExecution||{};
  out.protectedExecution.workDefinition=summary;
  if(out.executionReadiness){
    out.executionReadiness.downstream=out.executionReadiness.downstream||{};
    out.executionReadiness.downstream.workDefinitionStatus=summary.coverageStatus==='FULLY_COMPILED'
      ?'COMPILED'
      :'COMPILED_WITH_EXPLICIT_BLOCKERS';
  }
  return out;
}
function applySafeExecutionSummaries(entry,taskId,projection){
  return applySafeWorkDefinitionSummary(entry,taskId,applySafeDecompositionSummary(entry,taskId,projection));
}
...
export function buildPublicExecutionDepthProjection(args){
  ...
  if(entry.publicProjectionBundlePath)return applySafeExecutionSummaries(entry,args.taskId,loadPrecompiledPublicProjection(entry,args.taskId));
  ...
  return applySafeExecutionSummaries(entry,args.taskId,projection);
}
