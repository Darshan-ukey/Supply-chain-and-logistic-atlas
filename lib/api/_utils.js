...
const SAFE_PRESENTATION_CAPABILITIES=[
  'atlas.overview.read','atlas.operational.safe.read','atlas.readiness.read','atlas.provenance.safe.read','atlas.execution.summary.read'
];
const PROTECTED_PRESENTATION_CAPABILITIES=[
  'atlas.operational.full.read','atlas.provenance.full.read','atlas.work_decomposition.full.read','atlas.workdefinition.full.read','atlas.runtime.assigned.read'
];
const ALL_EXECUTION_CAPABILITIES=[
  ...
  ...SAFE_PRESENTATION_CAPABILITIES,...PROTECTED_PRESENTATION_CAPABILITIES,
  'atlas.binding.canonical.read','atlas.binding.own.read','atlas.binding.own.write','atlas.governance.propose','atlas.governance.approve','atlas.owner'
];
...
export async function capabilitiesForUser(user){
  ...
  const caps=new Set(['ATLAS_EXPLORE','EXECUTION_SUMMARY_VIEW',...SAFE_PRESENTATION_CAPABILITIES]);
  const meta=user.app_metadata||{};
  ...
  if(caps.has('WORKDEFINITION_VIEW'))caps.add('atlas.workdefinition.full.read');
  if(caps.has('ATLAS_CANONICAL_GOVERN')){caps.add('atlas.operational.full.read');caps.add('atlas.provenance.full.read')}
  return [...caps];
}
...
export const AtlasSafePresentationCapabilities=Object.freeze([...SAFE_PRESENTATION_CAPABILITIES]);
export const AtlasProtectedPresentationCapabilities=Object.freeze([...PROTECTED_PRESENTATION_CAPABILITIES]);
