...
export default async function handler(req,res){
 try{
  ...
  const {user}=await requireCapabilities(req,['atlas.workdefinition.full.read','WORKDEFINITION_VIEW'],{any:true});
  ...
  if(!definitions.length)return send(res,503,{ok:false,error:'Protected WorkDefinition store is empty. Run the private V2 WorkDefinition seed before using the protected execution layer.'});
  const registry={
  ...
  return send(res,200,{ok:true,admin:{id:user.id,email:user.email},registry},{'Cache-Control':'private, no-store, max-age=0','X-Atlas-Projection-Class':'EXECUTION_PROTECTED'});
 }catch(e){err(res,e)}
}
