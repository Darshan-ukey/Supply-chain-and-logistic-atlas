...
function evaluateState(manifest, targetState, config, ruleset) {
  ...
  const scopeNa = manifest.not_applicable_decisions.find(na =>
    na?.item_id === targetState &&
    ruleset.waiver_authorities.includes(na?.authority) &&
    isNonEmptyString(na?.decision_id) && !isFloating(na?.decision_version));
  if (scopeNa) {
    return buildResult({
      manifest, targetState, config, ruleset, result: 'NOT_APPLICABLE',
      blockers: [], predecessorProofs, monotonicityPassed: true,
      dependencyChainStatus: 'EVALUATED',
      ruleEvaluations: [{ rule_id: 'R-NA-001', rule_version: ruleset.ruleset_version, result: 'NOT_APPLICABLE', evidence_refs: [scopeNa.decision_id] }]
    });
  }
