...
## v0.6.6 Simulation / Interaction Runtime
- Simulation paths are deterministic outputs of the governed Road LTL graph.
- Gemini/other LLMs explain simulation state; they do not choose `PRECEDES`, branch, block, exception or recovery routes.
- Query Trust Gate is separate from document/evidence intake.
- UI actions are allowlisted and validated against real process/edge IDs.
- Handoff/data agents may expose explicit canonical objects and identifiers only; unmodeled payload fields remain `NOT_MODELED`.
- Risk statements preserve provenance and may not invent probability/severity/economics.
- Adaptive Information Density is presentation-only; hidden sections remain present and expandable.
